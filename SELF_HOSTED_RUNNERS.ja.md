...
Linux self-hosted runner はこの workflow では無効です。Ubuntu Docker validation は
hosted の Compatibility workflow で既に確認します。
...
self-hosted runner は、実際に重視する Windows / macOS マシン上の Docker Desktop
で動くことを証明したい場合に使います。
主目的は macOS / Windows 実機の確認です。
...
macOS / Windows のためです。Windows runner を試す場合は `windows` を明示します。
Linux self-hosted はこの workflow では無効です。
macOS と Windows を個別に通し、両方の runner が online の場合だけ最後にまとめて実行します。
